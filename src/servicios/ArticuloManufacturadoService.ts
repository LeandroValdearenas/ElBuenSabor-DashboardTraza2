...
export default class ArticuloManufacturadoService extends BackendClient<ArticuloManufacturado> {
    ...
    async buscarXDenominacion(busqueda:string, sucursalId: number): Promise<ArticuloManufacturado[]> {
        const response = await fetch(`${this.baseUrl}/busqueda/${sucursalId}/${busqueda}`);
        const data = await response.json();
        return data as ArticuloManufacturado[];
    }
}
